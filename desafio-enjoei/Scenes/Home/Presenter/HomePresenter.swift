...
class HomePresenter: NetworkManager {
        ...
        self.getProducts(urlString: "qdfq10ii") { [weak self] (response, error) in
            ...
            guard let products = response else {
                ...
                case .error(let message),
                    ...
                    self?.view?.displayMessageView(icon: .notFound, text: message, sizeIcon: 100, backgroundColor: .white)
                case .none(_): break
                }
                return
            }
            ...
            guard products.count > 0 else{
                self?.view?.displayMessageView(icon: .notFound, text: "No data found.", sizeIcon: 100, backgroundColor: .white)
                return
            }
    ...
    private func getProducts(urlString: String, completed: @escaping([Product]?, ServiceRequestError) -> Void) {
        request(endpoint: .products(urlString), method: .get, parameters: nil, compoundURL: false, success: { (response: ResponseProducts) in
            completed(response.products?.compactMap{$0}, .none("Success."))
        }, failure: { (error) in
            completed(nil, error)
        })
    }
    ...
    func configure(_ cell: HomeCell, for row:Int) {
        ...
        guard let size = product.size else {
            cell.display(description: product._price)
            cell.display(textColor: product._price, color: .mainRed)
            return
        }
        cell.display(description: "\(product._price) - tam \(size)")
        cell.display(textColor: product._price, color: .mainRed)
    }
